...
AboutWidget::AboutWidget(QWidget* parent) :QWidget(parent), d(new AboutWidgetPrivate(this))
{
...
#ifdef ENABLE_UPDATE_CHECK
	auto checkUpdateViaGithubBtn=new QPushButton("检查更新（github）");
	auto checkUpdateViaNjugitBtn=new QPushButton("检查更新（Via NJU git）");
	auto checkUpateLayout=new QHBoxLayout;
	checkUpateLayout->setAlignment(Qt::AlignHCenter|Qt::AlignTop);
	checkUpateLayout->addWidget(checkUpdateViaGithubBtn);
	checkUpateLayout->addSpacing(10);
	checkUpateLayout->addWidget(checkUpdateViaNjugitBtn);
	layout->addLayout(checkUpateLayout);
